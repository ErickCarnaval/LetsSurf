		...
		while (true){
			...
			for (Update update : updates) {
				...
				if(this.searchBehaviour==true){
					...
					List<City> cities = model.searchCity(update);
					if(!cities.isEmpty()){
						setControllerSearch(new ControllerSearchCity(model, this));
						for(City city: cities){
							sendResponse = bot.execute(new SendMessage(update.message().chat().id(),
									city.getCityName() + " - " + city.getCityUf() + ", id: " + city.getCityId()));
						}
						sendResponse = bot.execute(new SendMessage(update.message().chat().id(),"Type the city id! \nfonte 'CPTEC/INPE'"));
					}else{
						sendResponse = bot.execute(new SendMessage(update.message().chat().id(),"Sorry, city not found!"));							
					}
					this.searchBehaviour = true;

				} else {
					sendResponse = bot.execute(new SendMessage(update.message().chat().id(),"Welcome to SufingBot!"));
					sendResponse = bot.execute(new SendMessage(update.message().chat().id(),"Type city name or part of it to choose by city ID!"));
				}
			}
		}		
	}
